...
import { replaceNews, replaceSelected } from "../store/slices/newsSlice";
...
const SelectedPage = () => {
  ...
  useEffect(() => {
    ...
    const localSelected = JSON.parse(localStorage.getItem("selectedNews"));
    dispatch(replaceSelected(localSelected));
    const localNews = JSON.parse(localStorage.getItem("news"));
    if (localNews === null) return;
    dispatch(replaceNews(localNews));
  }, [dispatch]);
  return (
    <>
      <>
